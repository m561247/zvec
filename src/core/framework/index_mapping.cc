...
int IndexMapping::append(const std::string &id, size_t size) {
  ...
  // For compatibility, write offset relative to the content_offset,
  // and then restore the absolute offset while loading, i.e.,
  // init_index_mapping().
  segment->data_absolute_offset = index_size_;
  segment->data_size = 0;
  segment->data_crc = 0;
...
IndexMapping::Segment *IndexMapping::map(const std::string &id, bool warmup,
  ...
  if (!item->data()) {
    ...
    size_t offset = meta->data_absolute_offset;
...
int IndexMapping::flush(void) {
  ...
  for (auto iter = segments_.begin(); iter != segments_.end(); ++iter) {
    ...
    if (full_mode_ && copy_on_write_) {
      size_t off = item->meta()->data_absolute_offset;
