...
#include "hnsw_sparse_cut_streamer.h"
#include <iostream>
...
HnswSparseCutStreamer::HnswSparseCutStreamer() : entity_(stats_) {}

HnswSparseCutStreamer::~HnswSparseCutStreamer() {
  if (state_ == STATE_INITED) {
    this->cleanup();
  }
}

int HnswSparseCutStreamer::init(const IndexMeta &imeta,
                                const ailego::Params &params) {
  meta_ = imeta;
  meta_.set_streamer("HnswSparseCutStreamer", HnswSparseEntity::kRevision,
                     params);
...
int HnswSparseCutStreamer::cleanup(void) {
  if (state_ == STATE_OPENED) {
    this->close();
  }

  LOG_INFO("HnswSparseCutStreamer cleanup");
...
int HnswSparseCutStreamer::open(IndexStorage::Pointer stg) {
  LOG_INFO("HnswSparseCutStreamer open");
...
int HnswSparseCutStreamer::close(void) {
  LOG_INFO("HnswSparseCutStreamer close");
...
int HnswSparseCutStreamer::flush(uint64_t checkpoint) {
  LOG_INFO("HnswSparseCutStreamer flush checkpoint=%zu", (size_t)checkpoint);
...
int HnswSparseCutStreamer::dump(const IndexDumper::Pointer &dumper) {
  LOG_INFO("HnswSparseCutStreamer dump");
...
IndexStreamer::Context::Pointer HnswSparseCutStreamer::create_context(
    void) const {
...
HnswSparseCutStreamer::create_sparse_provider(void) const {
  LOG_DEBUG("HnswSparseCutStreamer create sparse provider");
  ...
  return SparseProvider::Pointer(
      new HnswSparseIndexProvider(meta_, entity, "HnswSparseCutStreamer"));
}

int HnswSparseCutStreamer::update_context(HnswSparseContext *ctx) const {
...
int HnswSparseCutStreamer::add_with_id_impl(uint32_t id,
  ...
  const float *sparse_real_query =
      reinterpret_cast<const float *>(sparse_query);
  std::vector<std::pair<float, uint32_t>> cut_query =
      Cut(sparse_count, sparse_indices, sparse_real_query, kDataCutOff);
  std::vector<uint32_t> cut_indices;
  std::vector<float> cut_values;
  for (int i = 0; i < cut_query.size(); i++) {
    cut_indices.push_back(cut_query[i].second);
    cut_values.push_back(cut_query[i].first);
  }

  std::string sparse_query_buffer;
  SparseUtility::TransSparseFormat(
      cut_query.size(), cut_indices.data(), cut_values.data(),
...
int HnswSparseCutStreamer::add_impl(uint64_t pkey, const uint32_t sparse_count,
                                    const uint32_t *sparse_indices,
                                    const void *sparse_query,
                                    const IndexQueryMeta &qmeta,
                                    Context::Pointer &context) {
...
int HnswSparseCutStreamer::search_impl(
...
int HnswSparseCutStreamer::search_impl(
  ...
  for (size_t q = 0; q < count; ++q) {
    ...
    const float *sparse_real_query =
        reinterpret_cast<const float *>(sparse_query_tmp);
    std::vector<std::pair<float, uint32_t>> cut_query = Cut(
        sparse_count[q], sparse_indices_tmp, sparse_real_query, kQueryCutOff);
    std::vector<uint32_t> cut_indices;
    std::vector<float> cut_values;
    for (int i = 0; i < cut_query.size(); i++) {
      cut_indices.push_back(cut_query[i].second);
...
//! Similarity search with sparse inputs
int HnswSparseCutStreamer::search_bf_impl(
    const uint32_t sparse_count, const uint32_t *sparse_indices,
    const void *sparse_query, const IndexQueryMeta &qmeta,
    IndexStreamer::Context::Pointer &context) const {
...
int HnswSparseCutStreamer::search_bf_by_p_keys_impl(
    const uint32_t sparse_count, const uint32_t *sparse_indices,
...
void HnswSparseCutStreamer::print_debug_info() {
  for (node_id_t id = 0; id < entity_.doc_cnt(); ++id) {
...
INDEX_FACTORY_REGISTER_STREAMER(HnswSparseCutStreamer);
