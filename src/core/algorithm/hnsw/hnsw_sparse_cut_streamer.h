...
class HnswSparseCutStreamer : public IndexStreamer {
 public:
  using ContextPointer = IndexStreamer::Context::Pointer;

  HnswSparseCutStreamer(void);
  virtual ~HnswSparseCutStreamer(void);

  HnswSparseCutStreamer(const HnswSparseCutStreamer &streamer) = delete;
  HnswSparseCutStreamer &operator=(const HnswSparseCutStreamer &streamer) =
      delete;
 ...
 protected:
  ...
  std::vector<std::pair<float, uint32_t>> Cut(const uint32_t sparse_count,
                                              const uint32_t *sparse_indices,
                                              const float *sparse_query,
                                              const float cut_ratio) const {
    std::vector<std::pair<float, uint32_t>> original_query, cut_query;
    for (int i = 0; i < sparse_count; i++) {
      original_query.push_back(
          std::make_pair(sparse_query[i], sparse_indices[i]));
    }
    sort(original_query.begin(), original_query.end());
    int cut_count = std::ceil(original_query.size() * cut_ratio);
    for (int i = std::max(0, (int)original_query.size() - cut_count);
         i < original_query.size(); i++) {
      cut_query.push_back(original_query[i]);
    }
    return cut_query;
  }
 ...
 private:
  ...
  const float kDataCutOff = 0.2;
  const float kQueryCutOff = 0.15;
};
